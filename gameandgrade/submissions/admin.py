...
class TaskAdmin(admin.ModelAdmin):
    fieldsets = [
                 (None,               {'fields': ['title']}),
                 ('Task Information', {'fields': ['descrip', 'fileName', 'xpVal', 'openTime', 'closeTime']}),
                 ]
...
class UploadAdmin(admin.ModelAdmin):
    def downloadUploads(self, request, queryset):
        ...
        for obj in queryset: #It does get the object I want
            ...
            archive = zip.writestr(obj.task.fileName, fileObj.read())
            fileObj.close
    ...
    list_display = ['task', 'userID', 'uploadTime', 'uploadURL']
...
#Without using these try/except cases, the site will complain that these models are being repeatedly registered
