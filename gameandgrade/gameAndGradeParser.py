...
@author: Lucy Moss, Eugene Shim

    'get_pattern()' attempts to match variables, module and method names to naming patterns
    
    'readable_output_list()' parses pylint output and formats it into a list of tuples for 'readable_output()'
    
    'readable_output()' formats the parsed pylint output for display in the 'views' module
...
#expresssions compiled for efficient use
#expression to catch when operators are not preceded/followed by a space
OPS = re.compile("([!][=]|[<][=]|[=][=]|[>][=]|[<]|[>]|[=]|[+][=]|[-][=]|[*][=]|[/][=]|[%])") 

#expression to parse the alert messages (excluding the alert number)
DIG = "(\d*)[,](\d*)([:](.*)[:]|[:])" 
...
def get_pattern(s):
    if s == "(([A-Z_][A-Z1-9_]*)|(__.*__))"or s=="(([A-Z_][A-Z0-9_]*)|(__.*__))":
        ...
        return ("Your variable is an inline variable, probably used in a generator.\n" +
                "Inlinve variables should use under_scores, not camelCase.\n" +
                "Inlinve variables should start with a Capital.\n" +
                "Inline variables can contain lowercase and numbers.\n")


def checkEmpty(infile):
    """
    Helper function that checks if file is empty of content
    """
...
def pylintEvaluate(inpath):
    """
    Creates a shell command to run ocpylint on the uploaded code
    """

    outputFile = inpath.split('/')
#    outputFile = outputFile[0:outputFile.rfind('prime.py')]
#    del outputFile[-1]
    del outputFile[-1]
    outputFile = '/'.join(outputFile)
    outputFile += "/pylintOutput.txt"
    
    if checkEmpty(inpath) == True: # If file is empty, just write that alert to the outpath file
        
        fout = open(outputFile, 'w')
        fout.write("File is empty")
        fout.close()
    else:
        ...
        p = subprocess.Popen(cmds,stdout=open(outputFile,'w'))  # This executes the command created and saves output
        stdout,stderr = p.communicate()  # Sends output to stdout to be saved.

#now returns as a list of triples!
#however the name has changed so as not to be overly confusing
def readable_output_list(output_file):
    try:
        ...
        #File is empty (custom)
        if "File is empty" in output:
            pretty_output.append((0, 0, "File is empty"))
        ...
        if re.search(r'Black listed name', output):
            ...
            for n in names:
                pretty_output.append((n[0],n[1],("The variable name you have used on line %s (%s) is reserved in Python.\n"%(n[0],n[4]) +
                                  "Choose a new name and be sure to change all instances of the variable.\n\n")))
            

        #Missing docstring (C0111)
        if re.search(r'Missing docstring', output):
            names = re.findall(r'%s Missing docstring' % (DIG), output)
            
            for n in names:
                pretty_output.append((n[0], n[1],('Please include a docstring on line %s.\n\n'%n[0])))
...
#                pretty_output += get_pattern(n[6]) + "\n"
        ...
        if re.search(r'Line too long', output):
            ...
            for n in names:
                pretty_output.append((n[0],n[1], ("Line %s is too long.\n"%n[0] +
                                  "The maximum length is 80 characters, but this line is %s characters.\n"%n[4] +
        ...
        if re.search(r'Too many lines in module', output):
            ...
            for n in names:
                pretty_output.append((n[0],n[1],("Module %s (line %s) has too many lines.\n"%(n[4], n[0]) +
        ...
        if re.search('More than one statement on a single line', output):
            names = re.findall(r'%s( More than one statement on a single line)'%DIG, output)

            for n in names:
                pretty_output.append((n[0],n[1],("On line %s you have more than one statement.\n"%n[0] +
        ...
        if re.search(r'Operator not preceded by a space\n', output):
            names = re.findall(r'%s( Operator not preceded by a space\n)(.*)%s(.*)'%(DIG,OPS), output)

            for n in names:
                pretty_output.append((n[0],n[1],"On line %s, the operator \"%s\" needs a space before it.\n\n"%(n[0],n[5])))


        #Operator not followed by space (C0323)
        if re.search('Operator not followed by a space', output):
            names = re.findall(r'%s( Operator not followed by a space\n)(.*)%s(.*)'%(DIG,OPS), output)

            for n in names:
                pretty_output.append((n[0],n[1],"On line %s, the operator \"%s\" needs a space after it.\n\n"%(n[0],n[5])))

        #Comma not followed by space (C0324)
        if re.search('Comma not followed by a space', output):
            names = re.findall(r'%s( Comma not followed by a space)'%DIG, output)

            for n in names:
                pretty_output.append((n[0],n[1],"On line %s, the comma needs a space after it.\n\n"%n[0]))

        #Syntax Error (E0001)
        if re.search('syntax', output):
            names = re.findall(r'%s(.*syntax.*)'%DIG, output)
            
            for n in names:
                
                pretty_output.append((n[0],n[1],"On line %s, there is a syntax error.\n\n "%n[0]))
        ...
        if re.search('Unrecognized file option', output):
            ...
            for n in names:
                pretty_output.append((n[0],n[1],"The option %s (line %s) is unrecognized.\n\n"%(n[4],n[0])))
        ...
        if re.search('Bad option value', output):
            ...
            for n in names:
                pretty_output.append((n[0],n[1],"The option %s (line %s) is not a valid option.\n\n"%(n[4],n[0])))
        ...
        if re.search('__init__ method is a generator', output):
            ...
            for n in names:
                pretty_output.append((n[0],n[1],("On line %s you defined the __init__ method as a generator.\n"%n[0] +
        ...
        if re.search('Explicit return in __init__', output):
            ...
            for n in names:
                pretty_output.append((n[0],n[1],("On line %s you included a return statement in the __init__ method.\n" +
        ...
        if re.search(' already defined line ', output):
            ...
            for n in names:
                pretty_output.append((n[0],n[1],("On line %s, you named a function, class, or method \"%s\".\n"%(n[0],n[3]) +
        ...
        if re.search('not properly in loop', output):
            ...
            for n in names:
                pretty_output.append((n[0],n[1],("On line %s, you used the loop keyword %s\n"%(n[0],n[3]) +
        ...
        if re.search('Return outside function', output):
            ...
            for n in names:
                pretty_output.append((n[0],n[1],("On line %s, you used the function keyword \"return\".\n"%n[0] +
        ...
        if re.search('Yield outside function', output):
            ...
            for n in names:
                pretty_output.append((n[0],n[1],("On line %s, you used the function keyword \"yield\".\n"%n[0] +
                                  "However, this was not inside of a function.\n" +
                                  "Check your indentation if you thought it was.\n\n")))
        ...
        if re.search('Return with argument inside generator', output):
            ...
            for n in names:
                pretty_output.append((n[0],n[1],("On line %s, you used \"return\" with the argument \"%s\".\n"%(n[0],n[4]) +
        ...
        if re.search ('An attribute inherited from .* hide.*', output):
            ...
            for n in names:
                pretty_output.append((n[0],n[1],("On line %s you defined a method which is hidden by an instance attribute of the ancestor class %s.\n"%(n[0],n[4]) +
                                  "If you do not want this attribute, consider re-structuring your inheritance.\n\n")))

        #Instance member called before assignment (E0203)
        if re.search('Access to member .* before its definition line .*', output):
            names = re.findall(r'%s( Access to member )(.*)( before its definition line )(.*)'%DIG, output)
            
            for n in names:
                pretty_output.append((n[0],n[1],("On line %s you tried to access the instance object %s.\n"%(n[0],n[4]) +
        ...
        if re.search('Method has no argument', output):
            ...
            for n in names:
                pretty_output.append((n[0],n[1],("On line %s you define a method with no argument.\n"%n[0] +
        ...
        if re.search(r'Method should have \"self\" as first argument', output):
            ...
            for n in names:
                pretty_output.append((n[0],n[1],("On line %s you define a method with arguments.\n"%n[0] +
        ...
        if re.search('Interface resolved to .* is not a class', output):
            ...
            for n in names:
                pretty_output.append((n[0],n[1],("On line %s you tried to implement the interface %s.\n"%(n[0],n[4]) +
        ...
        if re.search('Missing method .* from .* interface', output):
            ...
            for n in names:
                pretty_output.append((n[0],n[1],("Interface %s has method %s.\n"%(n[6],n[4]) +
        ...
        if re.search('Non ascii characters', output):
            ...
            for n in names:
                pretty_output.append((n[0],n[1],("On line %s you have non ASCII characters.\n"%n[0] +
        ...
        if re.search('Wrong encoding specified', output):
            ...
            for n in names:
                pretty_output.append((n[0],n[1],("On line %s, you have specified the encoding %s.\n"%(n[0],n[4]) +
        ...
        if re.search('Using variable .* before assignment', output):
            ...
            for n in names:
                pretty_output.append((n[0],n[1],("On line %s you referred to variable %s.\n"%(n[0],n[4]) +
        ...
        if re.search('No name .* in module', output):
            ...
            for n in names:
                pretty_output.append((n[0],n[1],("On line %s you tried to import %s from module %s.\n"%(n[0],n[4],n[6]) +
        ...
        if re.search('Bad except clauses order', output):
            ...
            for n in names:
                pretty_output.append((n[0],n[1],("On line %s, you ordered your \"exempt\" clauses i.n a poorly.\n"%n[0] +
        ...
        if re.search('while only classes, instances, or string are allowed', output):
            ...
            for n in names:
                pretty_output.append((n[0],n[1],("On line %s, you had a %s object.\n"%(n[0],n[4]) +
        ...
        if re.search('Bad first argument', output):
            ...
            for n in names:
                pretty_output.append((n[0],n[1],("On line %s you used \"super\" with an incorrect argument.\n"%n[0] +
        ...
        if re.search('.* has no .* member', output):
            ...
            for n in names:
                pretty_output.append((n[0],n[1],("On line %s you tried to use the %s member of %s.\n"%(n[1],n[7],n[5]) +
        ...
        if re.search('is not callable', output):
            ...
            for n in names:
                pretty_output.append((n[0],n[1],("On line %s you tried to call %s.\n"%(n[0],n[3]) +
        ...
        if re.search('Assigning to function call', output):
            ...
            for n in names:
                pretty_output.append((n[0],n[1],("On line %s you assign a valuable to the return value of a function.\n"%n[0]+
        ...
        if re.search('Statement seems to have no efect', output):
            ...
            for n in names:
                pretty_output.append((n[0],n[1],("The statement on line %s seems to have no effect.\n\n"%n[0])))
        ...
        if re.search('String statement has', output):
            ...
            for n in names:
                pretty_output.append((n[0],n[1],("The string on line %s has no effect.\n"%n[0] +
        ...
        if re.search('semicolon', output):
            ...
            for n in names:
                pretty_output.append((n[0],n[1],("On line %s you have an unnecessary semicolon.\n"%n[0] +
        ...
        if re.search('Bad indentation', output):
            ...
            for n in names:
                pretty_output.append((n[0],n[1],("On line %s you have an unexpected amount of indentation.\n"%n[0] +
        ...
        if re.search('Found indentation with', output):
            ...
            for n in names:
                pretty_output.append((n[0],n[1],("On line %s, you mixed tabs and spaces.\n"%n[0] +
                                  "Instead of tabs, use 4 spaces.\n\n")))

        
        #No alerts/errors (custom)
        if len(pretty_output) == 0:
            pretty_output.append((0,0,"No issues found."))
        
        return pretty_output


#returns as a single long string instead of a list of triples
def readable_output(output_file):
    readable = ""
    
    for a,b,c in readable_output_list(output_file):
        readable += c
    
    return readable
        

#print readable_output('output5.txt')
